...
final class HomeController extends AdminController
{
    ...
    public function index(): JsonResponse|JsonResource
    {
        $page = $this->basePage()->css($this->css())->body([
            Grid::make()->columns([
                ...
                Flex::make()->items([$this->pieChart(), $this->cube(),]),
            ]),
            Grid::make()->columns([
                $this->lineChart()->md(8),
                Flex::make()->className('h-full')->items([])->direction('column'),
            ]),
        ]);
    ...
    public function giteeWidget()
    {
        return Card::make()
            ...
            ->body(Custom::make()->id('osc-gitee-widget-tag')->onMount(<<<JS
// 隐藏 gitee-box
...
JS,
    ...
    {
        return Card::make()->className('h-96')->body(Wrapper::make()->className('h-full')->body([
    ...
    {
        return Card::make()->className('h-96')->body(Chart::make()->height(350)->config("{
...
series: [
    ...
    {
        return Card::make()->className('h-96 ml-4 w-8/12')->body(Html::make()->html(<<<HTML
<style>
